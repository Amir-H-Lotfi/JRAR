...
Public Class Decompressor
    ...
    Public Enum RarErrors
        ...
        ERAR_EWRITE = 19
        ERAR_SMALL_BUF = 20
        ERAR_CANCELLED = 21 ' this is called ERAR_UNKNOWN in unrar.h
    ...
    Const UCM_CHANGEVOLUMEW As Integer = 3
    Const UCM_NEEDPASSWORDW As Integer = 4
    ...
    Private Sub LoadFileList()
        ...
        Do Until iStatus <> 0
            ...
            If Ret <> RarErrors.ERAR_SUCCESS Then ProcessError(Ret)
            iStatus = RARReadHeaderEx(mRARHandle, uHeaderEx)
        Loop
    ...
    Public Shared Function RARCB(ByVal msg As Integer, ByVal userdata As Integer, ByVal P1 As IntPtr, ByVal P2 As Integer) As Integer
        ...
        Select Case msg
            ...
            Case UCM_NEEDPASSWORDW
                ...
                Dim f As New pwDlg
                f.ShowDialog()
                Dim enc As System.Text.Encoding = Encoding.Unicode ' little endian unicode
                Dim pwB As Byte() = enc.GetBytes(f.pw & Chr(0))
                P2 = f.pw.Length
                f.Close()
                Marshal.Copy(pwB, 0, P1, pwB.Length)
                Return +1 '??
    ...
    Private Sub ProcessError(ByVal ErroNum As RarErrors)
        ...
        Select Case ErroNum
            ...
            Case RarErrors.ERAR_BAD_DATA
                szMsg = "Bad archive, CRC error, header or data is damaged, or bad password"
            Case RarErrors.ERAR_BAD_ARCHIVE
                szMsg = "Bad volume, not a RAR archive"
            ...
            Case RarErrors.ERAR_EWRITE
                szMsg = "Write error"
            Case RarErrors.ERAR_SMALL_BUF
                szMsg = "Archive comment truncated, buffer too small"
            Case RarErrors.ERAR_CANCELLED
                szMsg = "Extract cancelled or unknown error"
            Case RarErrors.ERAR_MISSING_PASSWORD
                szMsg = "Password for encrypted file or header is not specified"
            Case RarErrors.ERAR_EREFERENCE
                szMsg = "Cannot open file source for reference record"
            Case RarErrors.ERAR_BAD_PASSWORD
                szMsg = "Wrong password is specified"
            Case RarErrors.ERAR_UNKNOWN_FORMAT
                szMsg = "Wrong password is specified"
            Case Else
                szMsg = "Error " & ErroNum
